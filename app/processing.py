...
class DojoAttendanceProcessor:
    ...
    def process(self, country, excel_file):
        ...
        for i in range(dojo_total_number.shape[0]):
            days = dojo_total_number['Esquema de asistencia a la fecha'].iloc[i]
            if days in dias_str:
                dias_int = int(days.split()[0])
                if dojo_total_number['Last week attendance'].iloc[i] >= dias_int:
                    cumplio.append('Si')
                    Follow_up.append('No_aplica')
                else:
                    cumplio.append('No')
                    if dojo_total_number['Last week attendance'].iloc[i] > 2:
                        if dias_int == 5:
        ...
        temporal_df = dojo_total_number[['Work Email','Full Name','Current Skill','Current Seniority','Glober Studio','Project Name','Last Entry Date','Dojo start date','Percentage','Country','City', 'Last week attendance', 'Esquema de asistencia a la fecha']].copy()
        ...
        new_row = {
            ...
            'Globers en DOJO': df_eligible_Dojo.shape[0],
            'Globers que tienen que asistir': dojo_total_number.shape[0],
            'Globers que cumplieron ': (temporal_df[temporal_df['cumplio'] == 'Si']).shape[0],
            'Revisión para seguimiento': None,
            '% de cumplimiento': (temporal_df[temporal_df['cumplio'] == 'Si']).shape[0] / dojo_total_number.shape[0] if dojo_total_number.shape[0] > 0 else 0,
